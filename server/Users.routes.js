...
router.get("/user",async (req, res) =>{
  if (req.isAuthenticated()) {
    ...
    UserService.findById(customerId, function (err, user) {
...
// update user details
router.put("/user",passport.authenticate('reauthenticate', { failureRedirect: '/reauthenticate' }), async (req, res) => {
  console.log(req.body)
 ...
 try {
  if (req.isAuthenticated()) {
    ...
    // Store new user in local DB
    await UserService.updateUser(customerId,newDetails);
    step=4;
    res.status(201)
    res.redirect("/");
